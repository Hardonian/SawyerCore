...
export class PredictionSystem {
  forecast(candidate: PredictionCandidate, analysis: PatternAnalysis): OutcomeForecast {
    ...
    const exactPattern = analysis.patterns.find(
      (pattern) => pattern.provider === candidate.provider && pattern.taskType === candidate.taskType
    );
    ...
    const predictedSuccessRate = round(
      exactPattern?.successRate
  ...
  private blockedForecast(candidate: PredictionCandidate, reason: string): OutcomeForecast {
    return {
      provider: candidate.provider,
      target: candidate.target,
      taskType: candidate.taskType,
  ...
  private riskReasons(
    pattern: ProviderTaskPattern | undefined,
...
function weightedAverage<T extends 'successRate' | 'averageLatencyMs' | 'averageCostUsd'>(
