...
function averageConfidence(scores: readonly DecisionCandidateScore[]): number {
  if (scores.length === 0) return 0;
  return Number((scores.reduce((sum, score) => sum + score.forecast.confidence, 0) / scores.length).toFixed(4));
}

function dedupeInsights(insights: ActionableInsight[]): ActionableInsight[] {
  const byId = new Map<string, ActionableInsight>();
