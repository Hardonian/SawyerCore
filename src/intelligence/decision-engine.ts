...
const DEFAULT_OBJECTIVE: DecisionObjective = {
...
export class DecisionEngine {
  ...
  private readonly predictionSystem: PredictionSystem;
  ...
  decide(input: DecisionInput): ExecutionDecision {
    ...
    const scores = input.candidates
      .map((candidate) =>
        this.scoreCandidate(
          candidate,
          this.predictionSystem.forecast(candidate, analysis),
          objective,
          input.task.latencyPreferenceMs,
          input.task.maxBudgetUsd
        )
      )
      .sort(
        (a, b) =>
    ...
    const actions = [
      ...
      ...alternatives
        ...
        .map(
          (score) =>
            `Prefer ${selected.candidate.provider} over ${score.candidate.provider}: predicted success improves by ${Math.round(
              (selected.forecast.predictedSuccessRate - score.forecast.predictedSuccessRate) * 100
            )} points.`
        )
    ];
  ...
  private scoreCandidate(
    ...
    const rawScore =
      forecast.predictedSuccessRate * objective.reliabilityWeight * 100
  ...
  private scoreReasons(forecast: OutcomeForecast, latencyScore: number, costScore: number): string[] {
    return [
...
function normalizeObjective(objective: Partial<DecisionObjective> | undefined): DecisionObjective {
