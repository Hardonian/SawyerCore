...
export class CostAwareProviderSelector {
  ...
  async select(
    ...
    const healthyProviders: RuntimeProvider[] = [];
    ...
    const localProviders = healthyProviders.filter(p =>
      p.target === 'LOCAL_CPU' || p.target === 'LOCAL_GPU' || p.target === 'LOCAL_NPU'
    );

    const remoteProviders = healthyProviders.filter(p =>
      p.target === 'VLLM_SERVER' || p.target === 'LITELLM_PROXY' || p.target === 'CLOUD_FALLBACK'
    );
   ...
   */
  private isComplexTask(task: AiTask): boolean {
