...
function checkDiskPressure(): 'LOW' | 'MEDIUM' | 'HIGH' {
  try {
...
async function probeGPU(): Promise<{ gpuAvailable: boolean; vramTotal?: number; vramAvailable?: number }> {
  try {
...
function probeBattery(): 'AC' | 'BATTERY' | 'LOW_POWER' | undefined {
  return 'AC';
}

function checkThermalThrottling(): boolean {
  return false;
}
