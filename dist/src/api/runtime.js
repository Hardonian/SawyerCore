...
import { UnifiedExecutionGraph } from '../system/execution-graph.js';
...
export async function getExecutionGraph() {
    if (!graphInstance) {
        ...
        graphInstance = new UnifiedExecutionGraph(providers, loadResult.config, new AuditLogger(new InMemoryAuditSink()), { defaultSignals: getDefaultSignals() });
    }
    return graphInstance;
}
export async function runTask(tenantId, taskInput) {
...
function normalizeTaskType(type) {
    if (type === 'completion')
        return 'chat';
    if (type === 'chat' || type === 'embedding' || type === 'classification' || type === 'summarization')
        return type;
    return 'chat';
}
...
function normalizeClassification(privacy) {
    if (privacy === 'private' || privacy === 'sensitive')
        return privacy;
    return 'public';
}
function privacyRequirementFor(classification) {
