...
export function generateDeterministicRecommendation(inventory) {
    ...
    const warnings = [];
    if (inventory.ramGb < 8)
        warnings.push('Low RAM detected; model choices are constrained.');
    if (inventory.batterySensitive)
        warnings.push('Battery-sensitive device: disable aggressive preloading.');
    return {
        profile,
        providerPriority,
        modelsByTask: {
...
export function maybeGenerateAiExplanation(base, enabled) {
    if (!enabled)
        return base;
    return {
        ...base,
