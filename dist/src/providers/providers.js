...
class OpenAiCompatibleProvider extends StubProvider {
    ...
    async healthCheck() {
        ...
        try {
            const models = await this.requestJson('GET', '/models');
            ...
            return { healthy: false, reason: `${this.name} unreachable: ${error.message}` };
        }
    }
    async runInference(task) {
        ...
        const body = await this.requestJson('POST', '/chat/completions', payload);
...
export class LlamaCppProvider extends StubProvider {
    ...
    async healthCheck() {
        try {
            ...
            return { healthy: false, reason: `llama.cpp unreachable: ${error.message}` };
        }
    }
    async runInference(task) {
        const startedAt = Date.now();
...
export function createProvidersFromConfig(config) {
    const providers = [];
    ...
    if (config.providers.vllm.enabled && config.toggles.enable_vllm && config.providers.vllm.endpoint) {
        providers.push(new VllmProvider({
