...
function loadConfig() {
    if (!existsSync('sawyer.config.json'))
        return safeDefaultConfig();
    return JSON.parse(readFileSync('sawyer.config.json', 'utf8'));
}
async function endpointCheck(name, url, timeoutMs) {
    if (!url) {
    ...
    try {
        ...
        return { name, ok: false, details: `unreachable (${error.message})`, fix: `Start ${name} service locally` };
    }
    finally {
        clearTimeout(timer);
    }
