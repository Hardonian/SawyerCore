...
export function createBillingRouter() {
    ...
    router.get('/usage/:tenantId', async (req, res) => {
        try {
            ...
            const endDate = req.query.endDate ? new Date(req.query.endDate) : new Date(new Date().getFullYear(), new Date().getMonth() + 1, 0);
    ...
    router.get('/payment-link', (req, res) => {
        try {
            ...
            const link = generatePaymentLink(priceId, tenantId, ref);
            res.json({ url: link });
        }
        catch (error) {
            res.status(500).json({ error: error.message });
