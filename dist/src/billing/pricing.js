...
export class PricingCatalog {
    static tierAssignments = new Map();
    static initialize() {
        for (const tier of defaultTiers) {
            pricingTiers.set(tier.name, tier);
        }
    }
    static registerTier(tier) {
        pricingTiers.set(tier.name, tier);
    }
    static getTier(name) {
    ...
    static assignTier(tenantId, tierName) {
    ...
    static getTierForTenant(tenantId) {
        const tierName = PricingCatalog.tierAssignments.get(tenantId);
        if (!tierName)
            return undefined;
        return pricingTiers.get(tierName);
    }
    static calculateCost(tenantId, eventType, quantity) {
        const tier = PricingCatalog.getTierForTenant(tenantId);
        if (!tier)
            return 0;
        const rate = tier.usageRates[eventType] ?? 0;
        return quantity * rate;
    }
    static checkLimits(tenantId, eventType, currentUsage) {
        const tier = PricingCatalog.getTierForTenant(tenantId);
        if (!tier?.limits)
            return { allowed: true };
        const limitMap = {
