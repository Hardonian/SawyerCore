...
describe('verify:determinism', () => {
    ...
    it('DeterministicEngine produces same runId for same inputs', async () => {
        ...
        const engine1 = new DeterministicEngine([provider], config, new AuditLogger(new InMemoryAuditSink()), {
            logFilePath: join(logDir, 'log1.jsonl'),
            clock: () => '2026-01-01T00:00:00.000Z'
        });
        const engine2 = new DeterministicEngine([provider], config, new AuditLogger(new InMemoryAuditSink()), {
    ...
    it('verifyOutputIntegrity detects tampering', async () => {
        ...
        mkdirSync(logDir, { recursive: true });
        const engine = new DeterministicEngine([provider], config, new AuditLogger(new InMemoryAuditSink()), {
