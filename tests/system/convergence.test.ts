...
import type { HistoricalRun } from '../../src/intelligence/index.js';
...
describe('verify:autonomy', () => {
  ...
  it('lets the autonomy loop execute queued work through the unified graph', async () => {
    ...
    const loop = new AutonomyLoop(
      ...
      eventBus,
      scheduleRegistry,
      { maxTasksPerTick: 2, healthCheckIntervalTicks: 1 }
    );
...
describe('verify:security', () => {
  ...
  it('fails closed when sensitive data only has a cloud execution path', async () => {
    ...
    const graph = new UnifiedExecutionGraph(
      [new CostedProvider('cloud-only', 'CLOUD_FALLBACK', 0.01, 220, false)],
      config,
...
function newGraph(providers: RuntimeProvider[]): UnifiedExecutionGraph {
  return new UnifiedExecutionGraph(
    providers,
    safeDefaultConfig(),
    new AuditLogger(new InMemoryAuditSink()),
    {
...
class CostedProvider implements RuntimeProvider {
  ...
  constructor(
    ...
    private readonly supportsPrivateData = true
  ) {
    this.name = name;
    this.target = target;
  }
...
function capabilityForTask(taskType: AiTask['type']): ProviderCapabilities['capabilities'][number] {
  const map: Record<AiTask['type'], ProviderCapabilities['capabilities'][number]> = {
    chat: 'chat',
