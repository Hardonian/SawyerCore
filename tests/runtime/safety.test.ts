...
import { SafetyController, type ExecutionDelegate } from '../../src/runtime/safety/safety-controller.js';
...
class SuccessDelegate implements ExecutionDelegate {
  async execute(
    _task: AiTask,
    _tenantId: string,
...
class FailingDelegate implements ExecutionDelegate {
  async execute(): Promise<never> {
    throw new Error('delegate failure');
  }
}

class DegradedDelegate implements ExecutionDelegate {
  async execute(): Promise<{ decision: string; result?: InferenceResult; reasons: string[]; degraded?: boolean }> {
    return {
      decision: 'DENY',
